...
class AbstractTemplateBasedEmiter:

    def __init__(self, template_file, output_file="TEMP", dir_path="tests\\output\\sources"):
        self._template = Environment(loader=FileSystemLoader(searchpath="./components/emiters/templates")).get_template(template_file)
        self._inputs = dict()
        self._full_path = os.path.join(os.getcwd(), dir_path, output_file + ".c2")

    def append(self, template_emiter):
        new_code = template_emiter.resolve()
        self._set_tail(new_code)
        full_code = self.resolve()
        self._set_header(full_code)

    def _set(self, placeholder, value):
        self._inputs[placeholder] = value
        return self

    def _set_header(self, value):
        self._set("DEFAULT_HEADER", value)

    def _set_tail(self, value):
        self._set("DEFAULT_TAIL", value)

    def _set_line(self, value):
        self._set("DEFAULT_LINE", value)

    def _get(self, placeholder):
        return self._inputs.get(placeholder, "")

    def resolve(self):
        output = self._template.render(self._inputs)
        self._set_tail("")
        self._set_line("")
        return output
    ...
    def add_to_line(self, value):
        self._set("DEFAULT_LINE", self._get("DEFAULT_LINE") + value)
